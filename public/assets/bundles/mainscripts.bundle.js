...
function skinChanger() {
    $(".choose-skin li").on("click", function() {
            ...
            var id = "theme-" + t.data("theme");
            $.ajax({
                type: "get",
                url: url + "/change-theme" + '/' + id,
        ...
        $(".rtl_mode input").on("change", function() {
            // this.checked ? $("body").addClass("rtl_active") : $("body").removeClass("rtl_active");
            if (this.checked) {
        ...
        $(".gradient_mode input").on("change", function() {
            // this.checked ? $(".theme-bg").addClass("gradient") : $(".theme-bg").removeClass("gradient");
            if (this.checked) {
            ...
            $.ajax({
                type: "get",
                url: url + "/change-theme-gradient" + '/' + grd,
                // data:{"_token": "{{ csrf_token() }}"},
                dataType: "json",
                success: function(response) {
...
function CustomJs() {
    $("#main-menu").metisMenu(),
        $("#left-sidebar .sidebar-scroll").slimScroll({ height: "calc(100vh - 65px)", wheelStep: 10, touchScrollStep: 50, color: "rgba(23,25,28,0.02", size: "3px", borderRadius: "3px", alwaysVisible: !1, position: "right" }),
        $(".btn-toggle-offcanvas").on("click", function() {
            $("body").toggleClass("offcanvas-active");
        }),
        $("#main-content").on("click", function() {
            $("body").removeClass("offcanvas-active"), $(".sticky-note").removeClass("open");
        }),
        ...
        $(".sidebar_light input").on("change", function() {
            // this.checked ? $(".sidebar").addClass("light_active") : $(".sidebar").removeClass("light_active");
            if (this.checked) {
                $(".sidebar").addClass("light_active");
                sidebar = 'light_active';
                $("#main-logo").attr('src', url + '/assets/images/Maintenance-logo-icon-black.png');
                $("#main-logo2").attr('src', url + '/assets/images/Maintenance-logo-test-black.png');
            } else {
                $(".sidebar").removeClass("light_active");
                sidebar = 'default-sidebar';
                $("#main-logo").attr('src', url + '/assets/images/Maintenance-logo-icon-white.png');
                $("#main-logo2").attr('src', url + '/assets/images/Maintenance-logo-test-white.png');

            }
            $.ajax({
                type: "get",
                url: url + "/change-theme-sidebar" + '/' + sidebar,
                // data:{"_token": "{{ csrf_token() }}"},
                dataType: "json",
                success: function(response) {
...
function SearchDiv() {
    $(".search-form input").focus(function() {
        ...
        $(".search-form input").blur(function() {
            $(this).val() || $(".recent_searche").hide("slow");
        });
}
$(function() {
        "use strict";
...
!(function() {
    ...
    (e.async = !0), (e.src = "https://embed.tawk.to/5e44175da89cda5a188591ec/1e0t1qduj"), (e.charset = "UTF-8"), e.setAttribute("crossorigin", "*"), t.parentNode.insertBefore(e, t);
})();